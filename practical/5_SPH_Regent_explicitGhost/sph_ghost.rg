...
task update_density(point_region: region(ispace(int1d), fs_point3d),
                    point_region_ghost: region(ispace(int1d), fs_point3d),
                    h: double)
where
    reads (point_region_ghost.x), reads (point_region_ghost.y), reads (point_region_ghost.z), reads(point_region_ghost.id),
...
do
    ...
    for p in point_region do
        ...
        for gp in point_region_ghost do
...
where
    reads (point_region_ghost.x), reads (point_region_ghost.y), reads (point_region_ghost.z), reads(point_region_ghost.id),
    reads (point_region_ghost.density),
    reads (point_region.x), reads (point_region.y), reads (point_region.z), reads(point_region.id),
...
-- insert artificial dependencies to ensure that this executes last
...
task main()
    ...
    -- build a region of all points for ghost access - this will be basically our "old" values
    -- On distributed execution runs this region will have a physical instance on every node
    -- therefore we trade huge data movements for easily achievable distribution.
    -- This is not a sophisticated distribution strategy for sph, but only a few lines of code in Regent
    var point_region_ghost = region(structured_particle_is, fs_point3d)
    ...
    copy(point_region, point_region_ghost)
    -- printer_points(point_region_ghost)
    -- Init finished
    ...
    for t = 0, cfg.iteration_count do
        for color in point_region_partition_small.colors do
            update_density(point_region_partition_small[color], point_region_ghost, cfg.h)
            -- c.printf(" Updated densities --\n\n")
        end

        copy(point_region.density, point_region_ghost.density)
        copy(point_region.accel, point_region_ghost.accel)
        
        for color in point_region_partition_small.colors do
            update_by_pressure_force(point_region_partition_small[color], point_region_ghost, cfg.h)
        ...
        -- update ghost region after every iteration
        copy(point_region.accel, point_region_ghost.accel)
        copy(point_region.vel, point_region_ghost.vel)
        copy(point_region.x, point_region_ghost.x)
        copy(point_region.y, point_region_ghost.y)
        copy(point_region.z, point_region_ghost.z)
    ...
    -- To make sure that the ghost_region is really updated after each iteration we can print out all our points and their respective ghosts 
    -- at the end of the simulation:
    -- c.printf("-- Printing point_region: \n\n")
    -- printer_points(point_region)
    -- c.printf("-- Printing point_region_ghost: \n\n")
    -- printer_points(point_region_ghost)
